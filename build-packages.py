...
sourceVersion = "0.1~a+%s" % (buildNumber,)
baseDir = os.getcwd()
tarball = 'vumi-wikipedia_%s.orig.tar.gz' % (sourceVersion,)
...
def configBuild(platform, platformver, dhmake):
    ...
    dch = """vumi-wikipedia (%s-0~ppa%s) %s; urgency=low
...
def prepareBuild():
    os.chdir(baseDir)
    os.system('mkdir -p dist/vumi-wikipedia')
    os.system('git archive HEAD -o dist/%s' % (tarball,))


def runBuild(platform, platformver, dhmake):
    os.chdir(os.path.join(baseDir, 'dist/vumi-wikipedia'))
